...
RC RecordPageHandler::update_record(const char *data, int offset, int data_size, const RID *rid)
{
  ...
  char* old_data = frame_->data() + page_header_->first_record_offset + (page_header_->record_size * rid->slot_num) + offset;
...
RC RecordFileHandler::update_record(const char *data, int data_size, int offset, const RID *rid)
{
  RC rc = RC::SUCCESS;
  ...
  rc = page_handler.update_record(data, offset, data_size, rid);
  // 📢 这里注意要清理掉资源，否则会与insert_record中的加锁顺序冲突而可能出现死锁
  // delete record的加锁逻辑是拿到页面锁，删除指定记录，然后加上和释放record manager锁
  // insert record是加上 record manager锁，然后拿到指定页面锁再释放record manager锁
  // page_handler.cleanup();
